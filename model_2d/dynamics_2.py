...
def computeOmegaDot(x, param, omega_cmd):
    ...
    x18 = param.r_1*x14
    x19 = param.m_0*x16 + param.m_1*x18 + param.m_l*x18 - param.r_1*x17*x5
    x20 = x8**2
    x21 = x9**2
    x22 = param.m_1*x17
    x23 = x14**2
    ...
    x27 = param.m_l*x24
    x28 = -x10*x27 - x12*x26
    x29 = param.g*param.m_l - x13*x17*x27 + x26*x3
    A = np.zeros((3, 3))
    b = np.zeros((3, 1))
    A[0,0] = param.m_0*x0 + param.m_1*x0 + param.m_l*x0 + param.theta_1 + x0*x5 + x4
    A[0,1] = param.theta_l + x1**2*x7 + x4 + x6**2*x7
    A[0,2] = x15 + x19
    A[1,0] = -1
    A[1,1] = 1
    A[1,2] = 0
    A[2,0] = x19
    A[2,1] = x15
    A[2,2] = param.m_0*x20 + param.m_1*x23 + param.m_l*x20*x21 + param.m_l*x23 + x17**2*x5 - x21*x22*x8
...
def computeContactForces(x, param, omega_cmd):
    ...
    x8 = x1*x7
    x9 = cos(phi)
    x10 = param.m_l*param.r_l
    x11 = phi_ddot*x10
    x12 = sin(phi)
    x13 = phi_dot**2*x10
    x14 = param.m_l*x5
    x15 = param.m_l*x0 + param.m_l*x4 + x11*x9 - x12*x13 - x14*x8
    x16 = param.m_1*x0 + param.m_1*x4 + x15 - x6*x8
    x17 = param.r_0 + param.r_1
    x18 = x17*x3*x7
    x19 = param.g*param.m_l - param.m_l*x18 + x11*x12 + x13*x9 + x14*x2
...
def computePositions(x, param):
    ...
    r_OS_0[2,0] = 0
    r_OS_1[0,0] = x0 - x1*sin(psi_0)
    r_OS_1[1,0] = param.r_0 + x1*cos(psi_0)
    r_OS_1[2,0] = 0
    return [r_OS_0, r_OS_1]
