    ...
    def compute_ctrl_input(self, x, beta_cmd, mode=StateIndex.ALPHA_2_IDX):
        ...
        if mode is StateIndex.ALPHA_2_IDX:
            beta_dot_cmd = saturate(self.kp *
                                    (beta_cmd -
                                     x[StateIndex.ALPHA_2_IDX]) -
                                    self.kd *
                                    x[StateIndex.ALPHA_DOT_2_IDX], self.beta_dot_max)
...
# param.m1 + param.m2 + param.m3) * param.r2**2) * beta_ddot / (param.l *
# param.m3 * np.sqrt(param.g**2 + beta_ddot**2 * param.r2**2)))
...
# param.m1) / (param.m2 + param.m3)) * beta_ddot / np.sqrt(beta_ddot**2 +
# param.g**2 / param.r2**2))
