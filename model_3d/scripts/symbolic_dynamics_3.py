...
R_IB2h = rot_axis3(-beta_z)
R_B2hB3 = rot_axis2(-beta_y) * rot_axis1(-beta_x)

R_B2B3 = rot_axis2(-phi_y) * rot_axis1(-phi_x)

# angular velocities
alpha_z_dot = symbols('alpha_z_dot')
...
ang_dot = Matrix([alpha_z_dot, psi_x_dot, psi_y_dot, beta_x_dot,
                  beta_y_dot, beta_z_dot, phi_x_dot, phi_y_dot])

omega_1 = Matrix([w_1x, w_1y, w_1z])
B2h_omega_2 = Matrix([w_2x, w_2y, w_2z])

R_IB3 = R_IB2h * R_B2hB3
B3_omega_3 = Matrix([phi_x_dot, 0, 0]) + R_B2B3.T * \
    Matrix([0, phi_y_dot, 0]) + R_B2hB3.T * B2h_omega_2

I_omega_3 = Matrix([0, 0, beta_z_dot]) + rot_axis3(-beta_z) * Matrix([0, beta_y_dot, 0]
                                                                     ) + rot_axis3(-beta_z) * rot_axis2(-beta_y) * Matrix([beta_x_dot, 0, 0])

sol = solve(I_omega_3 - R_IB3 * B3_omega_3, Matrix([beta_x_dot, beta_y_dot, beta_z_dot]))

ang_dot = ang_dot.subs([('beta_x_dot', sol[beta_x_dot]), ('beta_y_dot',
                                                          sol[beta_y_dot]), ('beta_z_dot', sol[beta_z_dot])])
...
if __name__ == '__main__':
    ...
    var = Matrix([psi_x, psi_y, x, y])
    var_dot = Matrix([psi_x_dot, psi_y_dot, x_dot, y_dot])

    v_OS2 = r_OS2.jacobian(var) * var_dot

    r_S2P2 = -r2 * e_S1S2

    v_P2 = v_OS2 + (R_IB2h * B2h_omega_2).cross(r_S2P2)
    ...
    om_i = [omega_1, B2h_omega_2, B3_omega_3]
    ...
    w_21 = ((R_IB2h * B2h_omega_2) - omega_1).dot(e_S1S2)
    ...
    M_i = [M1 - M12, R_IB2h.T * M12 - R_B2hB3 * R_B2B3.T * B2_Ty -
           R_B2hB3 * B3_Tx, R_B2B3.T * B2_Ty + B3_Tx]
