...
# angles
alpha_z, phi_x, phi_y, psi_x, psi_y = symbols("alpha_z phi_x phi_y psi_x psi_y")
...
omega_dot = Matrix([w_1_dot_z, psi_x_ddot, psi_y_ddot, w_2_dot_x, w_2_dot_y, w_2_dot_z, phi_x_ddot, phi_y_ddot])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="generation of symbolic dynamics of 3D Double Ball Balancer")
    parser.add_argument(
        "-d",
    ...
    parser.add_argument(
        "-p", "--print-dynamics", help="print common sub-expressions for dynamic model", action="store_true"
    )
    ...
    v_OS2 = (
        diff(r_OS2, x, 1) * x_dot
        + diff(r_OS2, y, 1) * y_dot
        + diff(r_OS2, psi_x, 1) * psi_x_dot
        + diff(r_OS2, psi_y, 1) * psi_y_dot
    )
    ...
    b_omega_3 = Matrix([phi_x_dot, 0, 0]) + rot_axis1(phi_x) * Matrix([0, phi_y_dot, 0]) + R_B2B3.T * b_omega_2
    ...
    R_IB2_dot = (R_IB2 * Matrix([[0, -w_2z, w_2y], [w_2z, 0, -w_2x], [-w_2y, w_2x, 0]])).reshape(9, 1)
    ...
    p_dot_i = [
        p.jacobian(omega) * omega_dot + p.jacobian(ang) * ang_dot + p.jacobian(R_IB2_flat) * R_IB2_dot for p in p_i
    ]
    ...
    M_i = [Matrix([0, 0, 0]) + M1 - M12, R_IB2.T * M12 - B2_Ty - R_B2B3 * B3_Tx, R_B2B3.T * B2_Ty + B3_Tx]

    # Spin
    omega_diff_i = [
        om_i[i].jacobian(omega) * omega_dot + om_i[i].jacobian(ang) * ang_dot + om_i[i].jacobian(R_IB2_flat) * R_IB2_dot
        for i in range(3)
    ]
    ...
    print("all T terms eliminated: {}".format(simplify(Matrix(dyn[:]).jacobian(Matrix([Tx, Ty]))) == zeros(8, 2)))
