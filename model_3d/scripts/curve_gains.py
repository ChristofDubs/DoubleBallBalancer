...
A = np.column_stack([a * b for a in [phi_x_motor] for b in [np.ones(omega_y.shape), np.abs(omega_y), omega_y**2]])
b = omega_x_cmd
...
A = np.abs(
    np.column_stack(
        [np.ones(omega_y_buckets.shape), omega_y_buckets, omega_y_buckets**2, omega_y_buckets**3, omega_y_buckets**4]
    )
)
...
A = np.column_stack(
    [
        a * b
        for a in [omega_z / omega_y, (omega_z / omega_y) * np.abs(omega_z / omega_y)]
